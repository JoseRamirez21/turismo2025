...
$id = intval($_GET['id'] ?? 0);
$provincia = $provinciaModel->getById($id);
$departamentos = $departamentoModel->getAll();

if (!$provincia) {
    die("❌ Provincia no encontrada.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($provinciaModel->update($id, $nombre, $id_departamento)) {
            header("Location: listar.php");
            exit;
        } else {
            $errors[] = "Error al actualizar la provincia.";
        }
    }
}

$pageTitle = "Editar Provincia";
  ...
  <div class="row">
    ...
    <main class="col-md-9 col-lg-10 px-md-4 py-4">
      ...
      <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-2">
        <h2 class="fw-bold text-primary">✏️ Editar Provincia</h2>
          ...
          <form method="POST">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="nombre" name="nombre" 
                     value="<?= htmlspecialchars($_POST['nombre'] ?? $provincia['nombre']) ?>" required>
            </div>
            <div class="mb-3">
              ...
              <select id="id_departamento" name="id_departamento" class="form-select" required>
                ...
                <?php foreach ($departamentos as $d): ?>
                  <option value="<?= $d['id_departamento'] ?>" 
                    <?= (($provincia['id_departamento'] ?? 0) == $d['id_departamento']) ? 'selected' : '' ?>>
