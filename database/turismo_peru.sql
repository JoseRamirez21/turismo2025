...
-- Tiempo de generación: 18-09-2025 a las 16:17:51
...
CREATE TABLE `count_request` (
  ...
  `tipo` enum('consulta','creacion','actualizacion','eliminacion') NOT NULL,
...
ALTER TABLE `count_request`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_count_token` (`id_token`);
...
ALTER TABLE `tokens`
  ...
  ADD KEY `fk_tokens_client` (`id_client_api`);
...
--
ALTER TABLE `count_request`
  ADD CONSTRAINT `fk_count_token` FOREIGN KEY (`id_token`) REFERENCES `tokens` (`id`) ON DELETE CASCADE;
...
--
ALTER TABLE `tokens`
  ADD CONSTRAINT `fk_tokens_client` FOREIGN KEY (`id_client_api`) REFERENCES `client_api` (`id`) ON DELETE CASCADE;
COMMIT;
